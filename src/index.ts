...
interface TabRecord {
  ...
  /** 输出环形缓冲：切标签/刷新页面后 SSE 重连回放用 */
  buffer: string[]
  bufferedBytes: number
  /** SSE 订阅者（每浏览器标签页一个 EventSource） */
  subscribers: Set<(chunk: string) => void>
  exitListeners: Set<(outcome: { exitCode: number | null; signal: NodeJS.Signals | null }) => void>
}
...
async function openTab(ctx: any, cwd?: string): Promise<TabRecord> {
  const subprocess = ctx.get('subprocess')
  if (!subprocess) throw new Error('subprocess service unavailable')
  const shell = userShell()
  const handle: TerminalHandle = await subprocess.spawnTerminal({
    argv: [shell, '-l'],
  ...
  handle.output.on('data', (raw: unknown) => {
    const chunk = typeof raw === 'string' ? raw : Buffer.from(raw as ArrayBufferLike).toString('utf8')
    ...
    for (const send of tab.subscribers) {
      try { send(chunk) } catch { /* 订阅者自行清理 */ }
    }
  })
  handle.done.then((outcome) => {
    tab.exited = true
    for (const notify of tab.exitListeners) {
      try { notify(outcome) } catch { /* ignore */ }
    }
  }).catch(() => {
    tab.exited = true
    for (const notify of tab.exitListeners) {
      try { notify({ exitCode: null, signal: null }) } catch { /* ignore */ }
    }
  })
...
async function closeTab(id: string): Promise<boolean> {
  ...
  try { await tab.handle.terminate() } catch { /* 已退出 */ }
  return true
}

function tabSummary(tab: TabRecord) {
  return { id: tab.id, title: tab.title, pid: tab.pid, createdAt: tab.createdAt, exited: tab.exited }
}

// ── RPC + SSE 路由 ───────────────────────────────────────────

const rpcHandlers = new Map<string, (args: any, ctx: any) => Promise<unknown>>()
let routesRegistered = false

function registerRoutes(ctx: any) {
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: (req: any, res: any) => {
      ...
      const onExit = (outcome: { exitCode: number | null; signal: NodeJS.Signals | null }) => {
      ...
      if (tab.exited) onExit({ exitCode: null, signal: null })
    },
...
export function apply(ctx: any) {
  ...
  rpcHandlers.set('tab.open', async (args: any) => {
    const tab = await openTab(ctx, args?.cwd)
    return tabSummary(tab)
  })
  rpcHandlers.set('tab.close', async (args: any) => {
    if (!args?.id) throw new Error('id required')
    const ok = await closeTab(args.id)
    if (!ok) throw new Error('tab not found')
    return true
  })
  rpcHandlers.set('tab.write', async (args: any) => {
  ...
  ctx.on('internal/service', (name: string) => {
    if (name === 'webServer' || name === 'httpServer') registerRoutes(ctx)
  })

  // 插件卸载 / DSH 退出：终止全部 PTY 会话
  ctx.effect(() => () => {
    for (const id of [...tabs.keys()]) void closeTab(id)
  })
}
