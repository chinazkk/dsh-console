...
const host = {
  call: (method, args) =>
    fetch('/plugins/dsh-console/rpc', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ method, args: args ?? null }),
    }).then((r) => r.json()).then((d) => { if (d && d.error !== undefined) throw new Error(String(d.error)); return d }),
}
...
function ensureTerminal(tabId) {
  ...
  es.onmessage = (e) => { try { term.write(JSON.parse(e.data)) } catch { /* ignore */ } }
  es.addEventListener('exit', () => {
    term.write('\r\n\x1b[90m── 会话已退出 ──\x1b[0m\r\n')
  })

  term.onData((data) => {
    host.call('tab.write', { id: tabId, data }).catch(() => { /* 断线时键入丢失，SSE 会重连 */ })
  })
...
function ConsolePanel() {
  ...
  React.useEffect(() => {
    ...
    void refreshTabs()
    return () => { /* 面板卸载不销毁终端：PTY 在 host 侧继续跑，回来靠回放还原 */ }
  }, [refreshTabs])

  // 激活标签：把对应终端 DOM 挂到容器
  React.useEffect(() => {
    const mount = mountRef.current
    if (!mount || !activeId) return
    for (const [id, entry] of termRegistry) {
      if (entry.div.parentNode === mount && id !== activeId) mount.removeChild(entry.div)
    }
    const entry = ensureTerminal(activeId)
    if (entry.div.parentNode !== mount) mount.appendChild(entry.div)
    entry.term.focus()
  }, [activeId, tabs])
  ...
  const closeTab = async (id) => {
    ...
    try { await host.call('tab.close', { id }) } catch { /* 已不在 */ }
  }
    ...
    h$('div', { className: 'dsc-tabbar' },
      tabs.map((t) =>
        h$('div', {
          ...
          h$('span', { className: 'dsc-tab-dot' + (t.exited ? ' dsc-dot-dead' : '') }),
          h$('span', { className: 'dsc-tab-title' }, t.title),
          h$('span', {
      ...
      h$('button', { className: 'dsc-newtab', title: '新建终端', onClick: openTab }, '+'),
      h$('div', { className: 'dsc-tabbar-spacer' }),
      connected ? null : h$('span', { className: 'dsc-offline' }, '⚠ 服务断开'),
      h$('button', { className: 'dsc-clear', title: '清屏', onClick: clearActive }, '清屏'),
    ),
    // ── 终端区 ──
    h$('div', { className: 'dsc-body' },
      activeId
        ? h$('div', { className: 'dsc-mount', ref: mountRef })
        : h$('div', { className: 'dsc-empty' },
            h$('div', { className: 'dsc-empty-icon' }, '>_'),
            h$('div', { className: 'dsc-empty-text' }, '暂无终端'),
            h$('button', { className: 'dsc-empty-btn', onClick: openTab }, '新建终端'),
          ),
    ),
    // ── 状态栏 ──
    h$('div', { className: 'dsc-status' },
      activeTab
  ...
  return {
    ...
    apply(ctx) {
      ...
      slots.inject('conversation.view', () => slots.register(
        { name: 'conversation.view', id: 'dsh-console', label: '控制台', order: 15 },
